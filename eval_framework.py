...
class UnslothModel(BaseModel):
    """Unsloth model wrapper for optimized inference"""
    
    def load(self, **kwargs):
        """Load Unsloth model - assumes model and tokenizer are already loaded"""
        # For Unsloth, we assume the model and tokenizer are passed as kwargs
        # since they're typically loaded externally with FastModel.from_pretrained
        self.model = kwargs.get('model_instance')
        self.tokenizer = kwargs.get('tokenizer_instance')
        
        if self.model is None or self.tokenizer is None:
            raise ValueError(
                "UnslothModel requires 'model_instance' and 'tokenizer_instance' "
                "to be passed as kwargs. Please load your model with "
                "FastModel.from_pretrained() and pass both instances."
            )
        
        logger.info(f"Using Unsloth model: {self.model_name}")
    ...
    def inference(self, prompt: str, max_tokens: int = 1024) -> Tuple[str, List[Dict]]:
        ...
        try:
            ...
            logger.error(f"Unsloth model inference error: {e}")
            error_messages = [
                {"role": "user", "content": prompt},
...
class CompetitionKit:
    ...
    def load_model(self, model_name: str, model_type: str = "auto", **kwargs):
        ...
        Args:
            ...
            model_type: Type of model ("chatgpt", "local", "unsloth", "custom", "auto" for auto-detection)
        ...
        if model_type == "chatgpt":
            ...
            self.model = UnslothModel(model_name)
    ...
    def _detect_model_type(self, model_name: str) -> str:
        ...
        if any(name in model_name.lower() for name in ["gpt", "chatgpt", "openai", 'o1', 'o3', 'o4']):
            return "chatgpt"
        elif any(name in model_name.lower() for name in ["unsloth", "medgemma", "gemma-3n"]):
            return "unsloth"
        else:
            return "local"
