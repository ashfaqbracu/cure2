...
class UnslothModel(BaseModel):
    """Unsloth model wrapper for fast inference and fine-tuning"""
    ...
    def load(self, **kwargs):
        ...
        try:
            ...
            dtype = kwargs.get('dtype', None)  # Auto-detect
            load_in_4bit = kwargs.get('load_in_4bit', True)  # Use 4bit quantization by default
            ...
            # Patch model for faster inference
            FastLanguageModel.for_inference(self.model)
            
            logger.info(f"Loaded Unsloth model: {self.model_name}")
            
        except ImportError as e:
            logger.error(f"Failed to import Unsloth dependencies. Install with: pip install unsloth")
    ...
    def inference(self, prompt: str, max_tokens: int = 1024) -> Tuple[str, List[Dict]]:
        ...
        try:
            ...
            logger.error("PyTorch is required for Unsloth inference")
            raise
            
        # Create messages for chat template
        messages = [
            {"role": "system", "content": "You are a helpful assistant specializing in medical knowledge."},
            {"role": "user", "content": prompt}
        ]
...
class CompetitionKit:
    ...
    def load_model(self, model_name: str, model_type: str = "auto", **kwargs):
        ...
        if model_type == "chatgpt":
            ...
            raise ValueError(f"Unknown model type: {model_type}. Supported types: chatgpt, local, unsloth, custom")
    ...
    def _detect_model_type(self, model_name: str) -> str:
        ...
        if any(name in model_name.lower() for name in ["gpt", "chatgpt", "openai", 'o1', 'o3', 'o4']):
            ...
            # For local models, default to Unsloth for better performance
            # Users can explicitly specify "local" if they prefer transformers
            return "unsloth"
